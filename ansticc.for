...
c     alternative method of quantifying source size - BWB
      ...
      PARAMETER(TLAB=0.04)
C
      PARAMETER(BIM1=2.52) ! 2.59) !3.61) 2.65
...
c      DATA fname/'x12OWMX'/nqu/3200/ !129xe197au dep soft no-comp-pro, 50 MeV
...
c     variables to calculate 2-particle rms - BWB 2009-06-08
      ...
      real(kind=REAL64) :: rc_x_rms   ! RMS of relative x
      real(kind=REAL64) :: rc_y_rms
      real(kind=REAL64) :: rc_z_rms
      real(kind=REAL64) :: rc_r_rms   ! RMS of relative r
...
c     One can cut in momentum xor energy, in the lab xor CMS frame
      real(kind=REAL64) :: PTOM(PCT),PTOMX(PCT) ! twice lab mo cut min, max in GeV/c
...
c      DATA KMN/0.000/,KMX/10.000/ ! no filter in KE
c      DATA KMN/0.013/,KMX/0.140/ !sn+sn
c      DATA KMN/0.020/,KMX/0.140/ !ar+sc,kr+nb
c      DATA KMN/0.01/,KMX/0.145/  !xe+au.
      DATA KMN/0.006/,KMX/0.080/  !FAUST, heilborn-verde 2016
...
c     gate in transverse momentum over mass
      parameter(ptmin=0.0)   ! minimum p_t/m (single particle)
...
c      PARAMETER(angmn=0,angmx=180) ! no filter - gate in lab theta
c      PARAMETER(angmn=12,angmx=62) ! xe129au197
...
c       parameter(angmn=21,angmx=65) !krnb100,120
      parameter(angmn=2.3,angmx=33.6) !FAUST detector, heilborn-verde 2016

      real ang, dang

      parameter(angcmn=0,angcmx=180) ! gate in CMS theta
c      parameter(angcmn=70,angcmx=110) ! gate in CMS theta
...
C
      ...
      INTEGER(2) IRX2(NSIZ),IRY2(NSIZ),IRZ2(NSIZ),ITC2(NSIZ),IRHR2(NSIZ)
C
      ...
      real (kind=REAL64) :: amij, angt, angtc, angtcd, angtd, bgcm
     &                    , cangt, cangtc, e2i, e2j, egate, eci
     &                    , ecij, ecj, ei, ej, gcm, p2ij, pci
     &                    , pci2, pcij, pcijk, pcj, pcjto2, pcjtok
     &  , pgate, pi2, pij, pijt, pijtk, pit, pitk, pito
     &                    , pito2, pitok, pjt, pjtk, pjto, pjto2, pjtok
     &                    , ptm, ptmi, ptmij, ptmj, ptot_theta_lab
     &                    , px2i, px2j, pxci, pxcj, pxi, pxij, pxj, py2i
     &                    , py2j, pyci, pycj, pyi, pyij
     & , pyj, pz2i, pz2j, pzci, pzcj, pzcij
     & , pzi, pzij, pzj, rcij, tcij, tij, val, xcij, xij
     & , ycij, ycms, yij, ylab, yxi, yyi, yzi, zcij, zij
C
      real (kind=REAL64) :: yxx !< maximum x-rapidity of any particle
     &                    , yxn !< minimum x-rapidity of any particle
     &                    , yyx !< maximum x-rapidity of any particle
     &                    , yyn !< minimum x-rapidity of any particle
...
C
      PBEAM=SQRT((EBEAM-AMB)*(EBEAM+AMB)) ! lab momentum per baryon of projectile
...
c     convert kinetic energies to 2*momentum
      if(uken)then
         do i=1,pct
            ptom(i)=2.*sqrt(kmn(i)*kmn(i)+2.*am1*kmn(i))
            ptomx(i)=2.*sqrt(kmx(i)*kmx(i)+2.*am1*kmx(i))
         enddo
      endif
c
      KF=1
...
!         ENDIF
      ...
      ! check for new limits of r-rapidities
      if(idic.eq.ipid.or.idic.eq.ipid2) then
       pxi=ixxi(ien)*1e-3
       pyi=iyyi(ien)*1e-3
       pzi=izzi(ien)*1e-3
       pi2=pxi**2+pyi**2+pzi**2
       ei=sqrt(pi2+masses(idic)**2)
...
!         Ip=IPO(i)
         ...
         PITOK=PITK+PZI*PZI
         pito=sqrt(pitok)
         EI=SQRT(AM1K+PITOK) ! total energy in lab
         ylab=atanh(pzi/ei)
         ptm=pit/am1
         CALL LOREN(0.0d0,0.0d0,-BGCM,GCM,PXI,PYI,PZI,EI
...
c      ! histNptm after-loop
     ...
     &                    +sorttime_tmin,' fm/c +/- ',0.5d0*sorttime_dt
       ...
       do ip=1,sorttime_tot(it)
        ...
        PITOK=PITK+PZI*PZI
        EI=SQRT(AM1K+PITOK) ! total energy in lab
        CALL LOREN(0.d0,0.d0,-BGCM,GCM,PXI,PYI,PZI,EI
     L            ,PXCI,PYCI,PZCI,ECI)
...
c     begin source calculation
      ...
      ien2=ien2-1
      WRITE(*,*)IEN, ien2
      IF(IEN.NE.0.and.ien2.ne.0)THEN
...
C
      ...
      real (kind=REAL64), intent(in) :: bgx,bgy,bgz,g,ec,pcx,pcy,pcz
      real (kind=REAL64), intent(out) :: px,py,pz,e

      real (kind=REAL64) :: bgpc, eg
